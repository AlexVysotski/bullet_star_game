...
for i in range(5): # создать 10 врагов
    mob = enemy_class.Enemy(height, wight)
    all_enemy.add(mob)
...
while running:  # запуск игрового цикла
    if len(all_enemy) < 5:  # если врагов меньше 10 , то создать новый эеземпляр класса врага
        mob = enemy_class.Enemy(height, wight)
        all_enemy.add(mob)

    screen.fill(BLACK)
    clock.tick(60)  # станавливаем количество игровыйх фреймов в секунду ( повторений while)
    ...
    if pygame.sprite.spritecollide(hero1, all_enemy, False): #остоновка програмы если спрайты столкнулись
        running = False
    if pygame.sprite.groupcollide(hero1.all_bullet, all_enemy, False, True):
        pass
    if pygame.sprite.groupcollide(hero2.all_bullet, all_enemy, False, True):
        pass
